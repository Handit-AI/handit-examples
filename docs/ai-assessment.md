...
* **Data quality issues** can lead to **misinformed business decisions**
  → Impact on decision-making processes and stakeholder trust
...
* Implementation approach: Add few-shot examples and chain-of-thought guidance
* **Target:** **Reduce mapping errors by 30%**

**B) Robust Edge Case Handling**

* Introduce explicit handling for common edge cases and ambiguous inputs
* Implementation details: Define specific scenarios and expected outputs
* **Target:** Increase accuracy in edge cases by 40%

**C) Deterministic Output Assurance**

* Strengthen guardrails to ensure consistent output format
* **Target:** Achieve 95% consistency in output structure
...
**Metrics we track:** Error rate reduction, processing time, output consistency
...
* [ ] Select target flows (document processing and schema mapping)
* [ ] Agree success metrics (error reduction, consistency improvement)
...
* **System Prompt:** Strong in format control but lacks examples for edge cases, which could improve mapping accuracy.
* **User Prompt:** Clear output contract but needs better context and rationale to guide the AI effectively.
